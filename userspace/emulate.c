...
extern int32_t __read (int32_t fd, void* buf, int32_t nbytes);
extern int32_t __write (int32_t fd, const void* buf, int32_t nbytes);
extern int32_t __close (int32_t fd);
void fake_function () {
    DO_CALL(halt,1 /* SYS_HALT */);
    DO_CALL(__read,3 /* SYS_READ */);
    DO_CALL(__write,4 /* SYS_WRITE */);
    DO_CALL(__close,6 /* SYS_CLOSE */);
...
execute (const uint8_t* command)
{
    ...
    if (1023 < strlen (command))
        return -1;
    buf[0] = '.';
    buf[1] = '/';
    strcpy (buf + 2, command);
