...
void create_init() {
    ...
    tasks[INIT]->usr_vid_table = page_tables[INIT][1];

    memset(tasks[INIT]->page_directory, PAGE_RW, TABLE_SIZE);
    memset(tasks[INIT]->kernel_vid_table, PAGE_RW, TABLE_SIZE);
    memset(tasks[INIT]->usr_vid_table, PAGE_RW, TABLE_SIZE);

    setup_vid(tasks[INIT]->page_directory, tasks[INIT]->kernel_vid_table, 0);
    setup_vid(tasks[INIT]->page_directory + TASK_VIDEO_OFFSET, tasks[INIT]->usr_vid_table, 1);
